...
func (obj *_SrMemberMgr) WithMerchantID(merchantID int64) Option {
...
func (obj *_SrMemberMgr) GetFromMerchantID(merchantID int64) (results []*SrMember, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(SrMember{}).Where("`merchant_id` = ?", merchantID).Find(&results).Error

	return
}

// GetBatchFromMerchantID 批量查找 商家id
func (obj *_SrMemberMgr) GetBatchFromMerchantID(merchantIDs []int64) (results []*SrMember, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(SrMember{}).Where("`merchant_id` IN (?)", merchantIDs).Find(&results).Error
