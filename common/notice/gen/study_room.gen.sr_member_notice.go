...
// FetchIndexBySrMemberNoticeMemberIDIndex  获取多个内容
func (obj *_SrMemberNoticeMgr) FetchIndexBySrMemberNoticeMemberIDIndex(memberID int64) (results []*SrMemberNotice, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(SrMemberNotice{}).Where("`member_id` = ?", memberID).Find(&results).Error
