...
func (obj *_DianpingAuthMgr) WithMerchantID(merchantID int64) Option {
...
func (obj *_DianpingAuthMgr) WithExpiresIn(expiresIn int64) Option {
	return optionFunc(func(o *options) { o.query["expires_in"] = expiresIn })
}

// WithExpiresAt expires_at获取 过期时间
func (obj *_DianpingAuthMgr) WithExpiresAt(expiresAt *time.Time) Option {
	return optionFunc(func(o *options) { o.query["expires_at"] = expiresAt })
}
...
func (obj *_DianpingAuthMgr) WithRemainRefreshCount(remainRefreshCount int) Option {
...
func (obj *_DianpingAuthMgr) GetFromMerchantID(merchantID int64) (result DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`merchant_id` = ?", merchantID).First(&result).Error

	return
}

// GetBatchFromMerchantID 批量查找 系统商家id
func (obj *_DianpingAuthMgr) GetBatchFromMerchantID(merchantIDs []int64) (results []*DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`merchant_id` IN (?)", merchantIDs).Find(&results).Error
...
func (obj *_DianpingAuthMgr) GetFromExpiresIn(expiresIn int64) (results []*DianpingAuth, err error) {
...
func (obj *_DianpingAuthMgr) GetBatchFromExpiresIn(expiresIns []int64) (results []*DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`expires_in` IN (?)", expiresIns).Find(&results).Error

	return
}

// GetFromExpiresAt 通过expires_at获取内容 过期时间
func (obj *_DianpingAuthMgr) GetFromExpiresAt(expiresAt *time.Time) (results []*DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`expires_at` = ?", expiresAt).Find(&results).Error

	return
}

// GetBatchFromExpiresAt 批量查找 过期时间
func (obj *_DianpingAuthMgr) GetBatchFromExpiresAt(expiresAts []*time.Time) (results []*DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`expires_at` IN (?)", expiresAts).Find(&results).Error
...
func (obj *_DianpingAuthMgr) GetFromRemainRefreshCount(remainRefreshCount int) (results []*DianpingAuth, err error) {
...
func (obj *_DianpingAuthMgr) GetBatchFromRemainRefreshCount(remainRefreshCounts []int) (results []*DianpingAuth, err error) {
...
// FetchUniqueByDianpingAuthMerchantIDUIndex primary or index 获取唯一内容
func (obj *_DianpingAuthMgr) FetchUniqueByDianpingAuthMerchantIDUIndex(merchantID int64) (result DianpingAuth, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(DianpingAuth{}).Where("`merchant_id` = ?", merchantID).First(&result).Error
