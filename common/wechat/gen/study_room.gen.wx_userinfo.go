...
func (obj *_WxUserinfoMgr) GetFromMemberID(memberID int64) (result WxUserinfo, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(WxUserinfo{}).Where("`member_id` = ?", memberID).First(&result).Error
...
// FetchUniqueByWxUserinfoMemberIDUIndex primary or index 获取唯一内容
func (obj *_WxUserinfoMgr) FetchUniqueByWxUserinfoMemberIDUIndex(memberID int64) (result WxUserinfo, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(WxUserinfo{}).Where("`member_id` = ?", memberID).First(&result).Error
